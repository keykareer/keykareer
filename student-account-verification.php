...
if(($nowtimestamp - $_SESSION['time']) > 300)  // 300 refers to 300 seconds or 5 minutes
{
    ...
    if ($otp == $_SESSION['session_otp']) 
    {
        ...
        $user_password =  $_SESSION['newPassword'] ;
        $user_id = $_SESSION['user_id'];
      
        $conn->query("UPDATE users SET user_password='$user_password' WHERE user_id=$user_id") or die("Couldn't connect to database");
      ...
      // Display the alert box 
        function function_alert($message) {
            echo "<script>alert('$message');  window.location='./student-account.php';</script>";
        }
...
if (isset($_POST['cancel'])){ //CANCEL BUTTON IF PRESSED
    unset($_SESSION['session_otp']);
    unset($_SESSION['user_verification']); //if unset, user cannot go to the verification page
    function function_alert($message) {
        // Display the alert box 
        echo "<script>alert('$message');  window.location='./student-account.php';</script>";
    }
...
<header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
 ...
 <span class="navbar-toggler-icon"></span>
  ...
  <div class="d-flex justify-content-center align-items-center container">
        ...
        <form action="student-account-verification.php" method="POST" >
