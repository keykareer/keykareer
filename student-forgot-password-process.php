...
if(isset($_POST['submit_btn'])){
    $email = $_POST['email'];
     ...
     $result = $conn->query("SELECT * FROM users WHERE user_email='$email'") or die("Couldn't connect to database"); //select all for checking
    if ($result){
        $row = $result->fetch_array();
        $user_email = $row['user_email'];


       if ($user_email==$email) { //check if email entered is in database
           

            header('location:student-forgot-password-verification.php'); //redirect to otp verification page
           ...
           //email sending
            ...
            $_SESSION['user_forgot_verification']=true;
